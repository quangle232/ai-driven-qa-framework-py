    ...
    if requested == "openai":
        if env.get("OPENAI_API_KEY"):
            return AiProviderConfig(
                "openai", env.get("OPENAI_MODEL", "gpt-4o-mini"), False, "AI_PROVIDER=openai"
            )
        return AiProviderConfig(
            "noop",
            "noop",
            True,
            "AI_PROVIDER=openai but OPENAI_API_KEY is not set — falling back to noop.",
        )

    # default: claude
    if env.get("ANTHROPIC_API_KEY"):
        return AiProviderConfig(
            "claude", env.get("ANTHROPIC_MODEL", "claude-opus-4-8"), False, "AI_PROVIDER=claude"
        )
    return AiProviderConfig(
        "noop", "noop", True, "ANTHROPIC_API_KEY is not set — falling back to noop."
    )
