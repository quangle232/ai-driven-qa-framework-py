...
class ActionKeyword:
    ...
    def _friendly_wait(self, element: Locator, state: str, timeout: float, action: str, selector: str) -> None:
        try:
    ...
    def wait_and_click(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
    ...
    def wait_and_fill(self, selector: str, value: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        element = self.page.locator(selector).nth(index)
        self._friendly_wait(element, "visible", timeout, "fill", selector)
        element.clear(timeout=timeout)
        element.fill(value, timeout=timeout)

    def select_by_value(self, selector: str, value: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        element = self.page.locator(selector).nth(index)
        element.wait_for(state="visible", timeout=timeout)
        element.select_option(value=value, timeout=timeout)

    def select_by_label(self, selector: str, label: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        element = self.page.locator(selector).nth(index)
        element.wait_for(state="visible", timeout=timeout)
    ...
    def wait_and_get_text(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> str:
        element = self.page.locator(selector).nth(index)
        element.wait_for(state="visible", timeout=timeout)
        return element.inner_text()

    def get_element_text(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> str:
    ...
    def wait_and_get_value(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> str:
        element = self.page.locator(selector).nth(index)
        element.wait_for(state="visible", timeout=timeout)
        return element.input_value() or ""

    def get_element(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> Locator:
        element = self.page.locator(selector).nth(index)
    ...
    def wait_for_element_hidden(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        self.page.locator(selector).nth(index).wait_for(state="hidden", timeout=timeout)

    # ---- assertions (auto-retrying via expect) ------------------------------

    def verify_element_visible(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT, reason: str = "") -> None:
        try:
            expect(self.page.locator(selector).nth(index)).to_be_visible(timeout=timeout)
        except AssertionError as err:
            label = self._friendly_label(selector)
            raise AssertionError(f"{reason or 'Expected visible'}: {label} not visible.\n  (selector: {selector})") from err

    def verify_element_not_visible(self, selector: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        expect(self.page.locator(selector).nth(index)).to_be_hidden(timeout=timeout)

    def verify_text_content(self, selector: str, text: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        expect(self.page.locator(selector).nth(index)).to_contain_text(text, timeout=timeout)

    def verify_value_content(self, selector: str, text: str, index: int = 0, timeout: float = DEFAULT_TIMEOUT) -> None:
        expect(self.page.locator(selector).nth(index)).to_have_value(re.compile(re.escape(text)), timeout=timeout)
