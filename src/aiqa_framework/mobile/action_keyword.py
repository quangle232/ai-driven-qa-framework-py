...
class MobileActionKeyword:
    ...
    def tap(self, accessibility_id: str) -> None:
        el = self._wait().until(EC.element_to_be_clickable((AppiumBy.ACCESSIBILITY_ID, accessibility_id)))
        el.click()

    def type(self, accessibility_id: str, text: str) -> None:
        el = self._wait().until(EC.visibility_of_element_located((AppiumBy.ACCESSIBILITY_ID, accessibility_id)))
        el.clear()
        el.send_keys(text)
    ...
    def wait_for_visible(self, accessibility_id: str) -> None:
        self._wait().until(EC.visibility_of_element_located((AppiumBy.ACCESSIBILITY_ID, accessibility_id)))

    def is_displayed(self, accessibility_id: str) -> bool:
        try:
            return self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, accessibility_id).is_displayed()
