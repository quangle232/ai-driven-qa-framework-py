...
from aiqa_framework.config.env import env_file_path, resolve_test_env
...
def run_checks() -> list[Check]:
    ...
    checks.append(("auth-setup", configured, "implement authenticate()" if not configured else "ok"))

    grpc_gen = Path("src/aiqa_framework/grpc/generated/game_pb2.py")
    checks.append(("grpc-stubs", grpc_gen.exists(), "run `poe proto-gen`" if not grpc_gen.exists() else "ok"))
