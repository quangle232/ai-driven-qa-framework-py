...
def detect_criticals(failures: list[FailureEvent]) -> list[CriticalEvent]:
    ...
    for trigger, words in _KEYWORDS.items():
        hits = [f for f in failures if any(w in f.error_message.lower() or w in f.test_id.lower() for w in words)]
        if hits:
            events.append(CriticalEvent(trigger=trigger, summary=f"{len(hits)} failure(s) match '{trigger}'"))
    ...
    if top_n / len(failures) >= 0.30 and top_n > 1:
        ...
        events.append(CriticalEvent(trigger="same-reason-cluster", summary=f"{pct}% share one error: {sample.error_message.splitlines()[0][:120]}"))
