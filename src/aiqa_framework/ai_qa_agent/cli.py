...
def _collect_to_disk():
    from aiqa_framework.ai_qa_agent.collectors.ci_metadata_collector import collect_ci_metadata
    from aiqa_framework.ai_qa_agent.collectors.pytest_report_reader import read_failures
...
def diagnose() -> None:
    ...
    from aiqa_framework.ai_qa_agent.agents.failure_diagnosis_agent import diagnose as run
    from aiqa_framework.ai_qa_agent.collectors.pytest_report_reader import read_failures
    from aiqa_framework.ai_qa_agent.reports.diagnosis_report_writer import write_diagnosis_md
...
def finalize() -> None:
    ...
    from aiqa_framework.ai_qa_agent.collectors.pytest_report_reader import read_failures
    from aiqa_framework.ai_qa_agent.reports.ci_summary_writer import write_ci_summary
    from aiqa_framework.ai_qa_agent.utils.run_id import resolve_run_id
...
def report_html() -> None:
    ...
    from aiqa_framework.ai_qa_agent.agents.failure_diagnosis_agent import diagnose as run
    from aiqa_framework.ai_qa_agent.collectors.pytest_report_reader import read_failures
    from aiqa_framework.ai_qa_agent.reports.stakeholder_html import write_stakeholder_html
...
def doctor() -> None:
    ...
    from aiqa_framework.ai_qa_agent.orchestration.doctor import run_checks
...
def guard(files: list[str] = typer.Option(None, "--files", help="Files to check")) -> None:
    ...
    from aiqa_framework.ai_qa_agent.analyzers.patch_guard import guard_file
...
def scan() -> None:
    ...
    index = {
        ...
        "api_services": [p.name for p in (root / "api/services").glob("*.py") if p.name != "__init__.py"],
        "screens": [p.name for p in (root / "mobile/screens").glob("*.py") if p.name != "__init__.py"],
