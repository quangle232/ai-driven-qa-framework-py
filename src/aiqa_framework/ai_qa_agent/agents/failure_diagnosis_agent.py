...
from aiqa_framework.ai_qa_agent.providers.llm import get_provider
from aiqa_framework.ai_qa_agent.schemas.diagnosis import ClusterDiagnosis, RunDiagnosis
...
_ENV_HINTS = ("timeout", "timed out", "connection", "econnrefused", "unavailable", "deadline", "reset by peer")
    ...
    for fingerprint, items in groups.items():
        ...
        clusters.append(
            ClusterDiagnosis(
                ...
                error_message=sample.error_message.splitlines()[0][:200] if sample.error_message else "",
                category=category,
                summary=summary,
            )
        )
