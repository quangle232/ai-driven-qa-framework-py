...
def register(router: Any, base: str = MOCK_BASE) -> None:
    ...
    router.post(f"{base}/users").mock(side_effect=lambda req: _resp(mock_store.create_user(_body(req))))
    router.post(f"{base}/auth/login").mock(side_effect=lambda req: _resp(mock_store.login(_body(req))))
