...
class ApiClient:
    ...
    def _send(
        ...
        *,
        ...
        url = path if path.startswith(("http://", "https://")) else f"{self._base}/{path.lstrip('/')}"
    ...
    @staticmethod
    def _assert_status(method: str, url: str, status: int, body: Any, expected: int | list[int] | None) -> None:
        if expected is None:
            return
